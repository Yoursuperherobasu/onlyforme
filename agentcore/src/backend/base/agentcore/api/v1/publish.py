...
This module provides endpoints for publishing AgentCore flows to OpenWebUI.
...
from agentcore.services.database.models.publish_record import (
    PublishRecord,
    PublishRecordCreate,
    PublishRecordRead,
    PublishStatusEnum,
)
from agentcore.services.openwebui_client import OpenWebUIClient
...
AGENTCORE_PIPE_FUNCTION_CODE = '''"""
title: AgentCore Flow Pipe
description: Execute AgentCore flows from OpenWebUI
...
class Pipe:
    class Valves(BaseModel):
        AGENTCORE_URL: str = Field(
            default="http://host.docker.internal:8765",
            description="AgentCore backend URL"
        )
        AGENTCORE_API_KEY: str = Field(
            default="",
    ...
    def __init__(self):
        ...
        self.pipelines = self.get_agentcore_flows()

    def get_agentcore_flows(self):
        """Fetch published flows from AgentCore backend."""
        try:
            agentcore_url = self.valves.AGENTCORE_URL.strip() if self.valves.AGENTCORE_URL else ""

            if not agentcore_url or not agentcore_url.startswith(("http://", "https://")):
                return [{"id": "error", "name": "Configure AGENTCORE_URL in Function Settings"}]

            # Fetch published flows from AgentCore
            url = f"{agentcore_url.rstrip('/')}/api/v1/publish/flows"
            headers = {}
            if self.valves.AGENTCORE_API_KEY:
                headers["x-api-key"] = self.valves.AGENTCORE_API_KEY
    ...
    def pipes(self):
        """Return list of available AgentCore flows as selectable models."""
        return self.get_agentcore_flows()
    ...
    async def pipe(self, body: dict, __user__: dict = None, __model__: dict = None):
        ...
        # Get AgentCore URL from valves
        agentcore_url = self.valves.AGENTCORE_URL.strip() if self.valves.AGENTCORE_URL else ""

        if not agentcore_url:
            return "Error: AGENTCORE_URL is not configured. Please set it in Function Settings."

        if not agentcore_url.startswith(("http://", "https://")):
            return f"Error: AGENTCORE_URL must start with http:// or https://. Current value: '{agentcore_url}'"

        # Prepare AgentCore request
        url = f"{agentcore_url.rstrip('/')}/api/v1/run/{flow_id}"
        ...
        try:
            async with httpx.AsyncClient(timeout=self.valves.REQUEST_TIMEOUT) as client:
                ...
                # Extract output from AgentCore's nested response structure
            ...
            return f"Error: AgentCore returned {e.response.status_code}"
        except Exception as e:
            return f"Error: {str(e)}"
'''


PIPE_FUNCTION_ID = "agentcore_flow_execution_pipe"
PIPE_FUNCTION_META = {
    "name": "AgentCore Flow Execution Pipe",
    "description": "Executes AgentCore flows as selectable models in OpenWebUI",
    "author": "AgentCore Team",
    "version": "1.0.8",
}
...
async def get_published_flows(
    ...
    fetch the list of available AgentCore flows.
        ...
        return [
            {
                "id": str(flow.id),
                "name": flow.name,
                "description": flow.description or f"AgentCore flow: {flow.name}",
            }
            for flow in flows
        ]
...
async def publish_to_openwebui(
    ...
    """Publish an AgentCore flow to OpenWebUI.
    ...
    try:
        ...
        await client.deploy_function(
            ...
            function_code=AGENTCORE_PIPE_FUNCTION_CODE,
