...
async def publish_notify_verify(
    *,
    ...
    # Write core deployment details to manifest.yaml (outside agentcore folder).
    # Failure here is non-fatal — the API response is always returned regardless.
    try:
        _status = record.status.value if hasattr(record.status, "value") else str(record.status)
        _is_active = "true" if record.is_active else "false"
        _manifest_lines = [
            f"# Generated by publish notify at {datetime.now(timezone.utc).isoformat()}",
            f"agent_id: {record.agent_id}",
            f"agent_name: {record.agent_name}",
            f"version_number: v{record.version_number}",
            "environment: prod",
            f"deployment_id: {record.id}",
            f"status: {_status}",
            f"is_active: {_is_active}",
        ]
        _manifest_path = Path(__file__).parents[6] / "manifest.yaml"
        _manifest_path.write_text("\n".join(_manifest_lines) + "\n", encoding="utf-8")
        logger.info(f"[PUBLISH_NOTIFY] manifest.yaml updated at {_manifest_path}")
