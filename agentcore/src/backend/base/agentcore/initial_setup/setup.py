...
from agentcore.base.constants import (
...
from agentcore.initial_setup.constants import STARTER_FOLDER_DESCRIPTION, STARTER_FOLDER_NAME
from agentcore.services.database.models.flow.model import Flow, FlowCreate
from agentcore.services.database.models.folder.constants import DEFAULT_FOLDER_NAME
from agentcore.services.database.models.folder.model import Folder, FolderCreate, FolderRead
from agentcore.services.database.models.user.crud import get_user_by_username
from agentcore.services.deps import get_settings_service, get_storage_service, get_variable_service, session_scope
from agentcore.template.field.prompt import DEFAULT_PROMPT_INTUT_TYPES
from agentcore.utils.util import escape_json_dump
...
async def load_flows_from_directory() -> None:
    """On agentcore startup, this loads all flows from the directory specified in the settings.

    NOTE: This feature is disabled with SSO authentication.
    Flows should be uploaded manually by authenticated users.
    """
    settings_service = get_settings_service()
    flows_path = settings_service.settings.load_flows_path
    if not flows_path:
        return
    # Disabled - SSO is enabled, no default superuser for automatic flow loading
    logger.warning("load_flows_from_directory is disabled with SSO. Flows should be uploaded manually.")
    return
...
async def load_bundles_from_urls() -> tuple[list[TemporaryDirectory], list[str]]:
    ...
    # Disabled - SSO is enabled, cannot load flows without authenticated user
    logger.warning("load_flows_from_urls flow loading is disabled with SSO. Components will still be loaded.")

    # Only load components, not flows (flows need authenticated user)
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as zfile:
            ...
            for filename in zfile.namelist():
                ...
                for dir_name in dir_names:
                    # Only load components, not flows
                    if path.is_relative_to(f"{dir_name}components/"):
                        if temp_dir is None:
