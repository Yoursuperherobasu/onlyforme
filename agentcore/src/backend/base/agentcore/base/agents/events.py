...
def _extract_output_text(output: str | list) -> str:
    ...
    # Handle multi-element lists (e.g. Google Gemini returns text + extras/signature blocks)
    text_parts: list[str] = []
    for item in output:
    ...
    # All items were tool_use / partial_json / non-text — return empty
    # instead of crashing (e.g. pure tool-call responses from some LLMs)
    return ""
