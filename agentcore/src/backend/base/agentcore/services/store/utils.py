...
if TYPE_CHECKING:
    from agentcore.services.store.schema import ListComponentResponse
...
# Get the latest released version of agentcore (https://pypi.org/project/agentcore/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/agentcore/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.opt(exception=True).debug("Error getting the latest version of agentcore from PyPI")
        return None
