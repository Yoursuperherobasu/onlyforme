...
# A single very permissive pattern: any action verb anywhere in the
# message followed (with up to 3 filler words like "me", "the", "my",
# "all") by an email/mail/inbox/message noun is an email-read intent.
# Also matches "inbox" on its own and "how many emails".
_EMAIL_READ_PAT = re.compile(
    r"("
    r"(?:show|list|get|fetch|check|read|open|display|give|see|view|pull)\b"
    r"(?:\s+\w+){0,3}\s+"
    r"(?:email|mail|inbox|message)s?"
    r"|"
    r"(?:recent|latest|last|unread|new|today'?s?|yesterday'?s?)\s+"
    r"(?:email|mail|message)s?"
    r"|"
    r"how\s+many\s+(?:email|mail|message)s?"
    r"|"
    r"emails?\s+(?:from\s+)?(?:today|yesterday|this\s+week|last\s+\d+\s+days?)"
    r"|"
    r"\binbox\b"
    r")",
    re.IGNORECASE,
