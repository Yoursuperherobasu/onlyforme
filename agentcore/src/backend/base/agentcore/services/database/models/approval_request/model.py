...
class ApprovalRequestBase(SQLModel):
    ...
    agent_publish_id: UUID = Field(
        sa_column=Column(
            Uuid(),
            ForeignKey("agent_deployment_prod.id", use_alter=True, name="fk_approval_agent_publish_id"),
            nullable=False,
        ),
    ...
    reviewer_id: UUID | None = Field(
        ...
        foreign_key="user.id",
        nullable=True,
        description="User who actually reviewed (may differ from request_to)",
    )
...
class ApprovalRequest(ApprovalRequestBase, table=True):  # type: ignore[call-arg]
    ...
    reviewer: Optional["User"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[ApprovalRequest.reviewer_id]"},
    )
    deployment_prod_record: Optional["AgentDeploymentProd"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[ApprovalRequest.agent_publish_id]"},
    )

    __table_args__ = (
        Index("ix_approval_agent_publish_id", "agent_publish_id"),
        Index("ix_approval_reviewer_id", "reviewer_id"),
