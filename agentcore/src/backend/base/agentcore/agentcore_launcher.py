...
def main():
    """Launches agentcore with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from agentcore.__main__ import main as agentcore_main

        agentcore_main()


def _launch_with_exec():
    """Launch agentcore by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "agentcore.__main__"] + sys.argv[1:])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec agentcore: {e}", file=sys.stderr)
        sys.exit(1)
