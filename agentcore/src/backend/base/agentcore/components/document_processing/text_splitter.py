...
INPUT:  list[Data] from Gemini OCR Extractor (or any source)
OUTPUT: list[Data] — smaller chunks with full metadata lineage

Strategies:
  - Recursive: splits on paragraph → newline → sentence → space (best general purpose)
  - Sliding Window: fixed-size overlapping windows (uniform sizes)

Metadata preserved on every chunk:
  source_file, page_number, file_type, extraction_method, chunk_id, chunk_index, kb_id

Connects:  [Gemini OCR Extractor] → [Text Splitter] → [Pinecone]
"""
...
class TextSplitterNode(Node):
    ...
    inputs = [
        HandleInput(
            ...
            input_types=["Data"],
            info="Input documents from Gemini OCR Extractor (or any list[Data] source).",
            is_list=True,
        ),
        StrInput(
            name="kb_id",
    ...
    # ── OUTPUT DEFINITION ─────────────────────────────────────
    # Bound to the split_documents() method — this is what produces
    # the output port on the node in the LCNC canvas.
    outputs = [
        Output(
            display_name="Chunks",
            name="chunks",
