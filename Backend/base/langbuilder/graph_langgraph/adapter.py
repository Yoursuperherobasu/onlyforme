"""Main LangGraph adapter for LangBuilder - fully independent implementation."""
...
from langbuilder.graph_langgraph.constants import Finish
from langbuilder.graph_langgraph.nodes import create_node_function
from langbuilder.graph_langgraph.state import LangBuilderState
from langbuilder.graph_langgraph.utils import (
...
if TYPE_CHECKING:
    ...
    from langbuilder.schema.schema import InputValueRequest
...
class LangGraphAdapter:
    ...
    def _build_langgraph_workflow(self) -> None:
        ...
        # Create StateGraph
        self.workflow = StateGraph(LangBuilderState)
        
        # Add nodes to workflow
        for vertex in self.vertices:
            node_func = create_node_function(vertex)
            self.workflow.add_node(vertex.id, node_func)
            logger.debug(f"Added node: {vertex.id} ({vertex.display_name})")
    ...
    async def end_all_traces_in_context(self, error: Exception | None = None) -> None:
        ...
        if self.tracing_service and not self.tracing_service.deactivated:
            ...
            outputs["timestamp"] = datetime.now(timezone.utc).isoformat()
    ...
    async def arun(
        ...
        *,
        ...
        """
        from langbuilder.graph_langgraph.schema import RunOutputs
        from langbuilder.services.deps import get_chat_service
    ...
    async def get_next_runnable_vertices(self, lock, vertex: LangGraphVertex, cache: bool = False) -> list[str]:
        ...
        Returns:
            List of next runnable vertex IDs
        """
        v_id = vertex.id
        ...
        async with lock:
            ...
            logger.info(f"🔍 GET_NEXT_RUNNABLE: vertex={v_id}, successors={v_successors_ids}, next_runnable={next_runnable_vertices}, inactivated={self.inactivated_vertices}")
    ...
    async def end_all_traces_in_context(self, error: Exception | None = None) -> None:
        """End all traces (compatibility method).
        
        Args:
            error: Optional error that occurred
        """
        # No-op for now, tracing handled elsewhere
        pass
    ...
    async def astep(
        ...
        from langbuilder.services.chat.service import get_chat_service
        chat_service = get_chat_service()
    ...
    def mark_branch(self, vertex_id: str, state: str, output_name: str | None = None) -> None:
        ...
        """
        from langbuilder.graph_langgraph.utils import build_adjacency_maps
