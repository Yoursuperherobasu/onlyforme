...
from langbuilder.custom.utils import abuild_custom_components, create_component_template
...
def _patch_langchain_modelprofile() -> None:
    """Provide stub ModelProfile types for langchain_core if missing.

    Recent langchain-core releases removed ModelProfile/ModelProfileRegistry, but
    langchain-google-genai still imports them. To avoid import crashes while loading
    components, install lightweight stub classes when absent. This keeps component
    discovery running; functionality relying on model profiles will be limited but
    import-time errors are prevented.
    """

    try:  # pragma: no cover - defensive shim
        from langchain_core import language_models as _lm

        if not hasattr(_lm, "ModelProfile"):
            class ModelProfile(dict):
                pass

            class ModelProfileRegistry(dict):
                pass

            class ModelSpec(dict):
                pass

            _lm.ModelProfile = ModelProfile
            _lm.ModelProfileRegistry = ModelProfileRegistry
            _lm.ModelSpec = ModelSpec
    except Exception:
        # If langchain_core itself cannot be imported, let the normal error path handle it
        logger.debug("Could not patch langchain_core ModelProfile stubs", exc_info=True)
...
async def import_langbuilder_components():
    """Asynchronously discovers and loads all built-in Langbuilder components with module-level parallelization.

    Scans the `langbuilder.components` package and its submodules in parallel, instantiates classes that are subclasses
    ...
    """
    # Ensure langchain_google_genai can import on langchain-core versions that removed ModelProfile
    _patch_langchain_modelprofile()
    modules_dict = {}
    try:
        import langbuilder.components as components_pkg
    except ImportError as e:
        logger.error(f"Failed to import langbuilder.components package: {e}", exc_info=True)
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    try:
        ...
        return None
    # Extract the top-level subpackage name after "langbuilder.components."
    # e.g., "langbuilder.components.Notion.add_content_to_page" -> "Notion"
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Langbuilder
    ...
    if component_cache.all_types_dict is None:
        ...
        langbuilder_components = await import_langbuilder_components()
        custom_components_dict = await _determine_loading_strategy(settings_service)

        # merge the dicts
        component_cache.all_types_dict = {
            **langbuilder_components["components"],
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from langbuilder.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from langbuilder.custom.utils import build_custom_components
