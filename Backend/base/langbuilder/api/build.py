...
from langbuilder.api.disconnect import DisconnectHandlerStreamingResponse
...
from langbuilder.schema.message import ErrorMessage
from langbuilder.schema.schema import OutputValue
from langbuilder.services.database.models.flow.model import Flow
from langbuilder.services.deps import get_chat_service, get_telemetry_service, session_scope
...
async def generate_flow_events(
    *,
    ...
    async def create_graph(fresh_session, flow_id_str: str, flow_name: str | None) -> Graph | LangGraphAdapter:
        ...
        # Use LangGraph by default (use_langgraph=True)
    ...
    def sort_vertices(graph: Graph | LangGraphAdapter) -> list[str]:
        try:
            # LangGraphAdapter uses its own sorting logic
            if isinstance(graph, LangGraphAdapter):
                # Get first layer from in_degree_map (vertices with no dependencies)
                first_layer = [vid for vid, degree in graph.in_degree_map.items() if degree == 0]
                logger.info(f"🔍 SORT_VERTICES: in_degree_map={graph.in_degree_map}")
                logger.info(f"🔍 SORT_VERTICES: first_layer={first_layer}")
                return first_layer if first_layer else list(graph.vertex_map.keys())[:1]
            else:
