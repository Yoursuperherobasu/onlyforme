...
class Data(BaseModel):
    ...
    def validate_data(cls, values):
        ...
        if not isinstance(values["data"], dict):
            msg = (
                ...
                " This will raise an error in version langbuilder==1.3.0."
            )
            logger.warning(msg)
    ...
    def to_lc_message(
        ...
        if sender == MESSAGE_SENDER_USER:
            if files:
                from langbuilder.schema.image import get_file_paths
    ...
    def filter_data(self, filter_str: str) -> Data:
        ...
        """
        from langbuilder.template.utils import apply_json_filter

        return apply_json_filter(self.data, filter_str)

    def to_message(self) -> Message:
        from langbuilder.schema.message import Message  # Local import to avoid circular import
...
def custom_serializer(obj):
    if isinstance(obj, datetime):
        ...
        return utc_date.strftime("%Y-%m-%d %H:%M:%S.%f %Z")
