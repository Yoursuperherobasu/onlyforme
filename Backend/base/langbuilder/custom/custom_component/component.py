...
from langbuilder.base.tools.constants import (
...
# from langbuilder.graph.state.model import create_state_model
# Lazy import to avoid circular dependency
# from langbuilder.graph.utils import has_chat_output
from langbuilder.helpers.custom import format_type
from langbuilder.memory import astore_message, aupdate_messages, delete_message
from langbuilder.schema.artifact import get_artifact_type, post_process_raw
from langbuilder.schema.data import Data
from langbuilder.schema.message import ErrorMessage, Message
...
class Component(CustomComponent):
    ...
    async def _build_tools_metadata_input(self):
        try:
            from langbuilder.io import ToolsInput
        except ImportError as e:
            msg = "Failed to import ToolsInput from langbuilder.io"
    ...
    async def _send_message_event(self, message: Message, id_: str | None = None, category: str | None = None) -> None:
        if hasattr(self, "_event_manager") and self._event_manager:
            ...
            # Debug logging to track event order
            sender = data_dict.get("sender", "unknown")
            timestamp = data_dict.get("timestamp", "no-timestamp")
            logger.info(f"📤 SEND_MESSAGE_EVENT: sender={sender}, timestamp={timestamp}, category={category}")
            ...
            logger.info(f"📤 SEND_MESSAGE_EVENT DONE: sender={sender}")
