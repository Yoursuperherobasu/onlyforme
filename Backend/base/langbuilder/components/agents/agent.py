        ...
        try:
            ...
            if self.add_current_date_tool:
                if not isinstance(self.tools, list):  # type: ignore[has-type]
                    self.tools = []
                # Check if a tool with same name already exists (avoid duplicates for Gemini)
                existing_tool_names = {getattr(t, "name", None) for t in self.tools if hasattr(t, "name")}
                current_date_tool = (await CurrentDateComponent(**self.get_base_args()).to_toolkit()).pop(0)
                if not isinstance(current_date_tool, StructuredTool):
                    msg = "CurrentDateComponent must be converted to a StructuredTool"
                    raise TypeError(msg)
                if current_date_tool.name not in existing_tool_names:
                    self.tools.append(current_date_tool)
            # note the tools are not required to run the agent, hence the validation removed.

            # Deduplicate tools by name (some models like Gemini don't allow duplicate function names)
            if self.tools:
                ...
                for tool in self.tools:
                    ...
                    if tool_name and tool_name not in seen_names:
                        ...
                        unique_tools.append(tool)  # Keep tools without names
