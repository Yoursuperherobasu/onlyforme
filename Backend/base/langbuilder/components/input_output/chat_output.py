...
class ChatOutput(ChatComponent):
    ...
    async def message_response(self) -> Message:
        ...
        if isinstance(self.input_value, Message):
            ...
            # Update message text
            message.text = text
            # Preserve existing sender_name if message already has one, otherwise use ChatOutput's setting
            if not message.sender_name or message.sender_name == "AI":
                message.sender_name = self.sender_name
            # Preserve existing sender if message already has one
            if not message.sender:
                message.sender = self.sender
        else:
            message = Message(text=text)
            # Set message properties for new messages
            message.sender = self.sender
            message.sender_name = self.sender_name
