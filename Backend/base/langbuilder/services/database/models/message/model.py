...
from langbuilder.schema.properties import Properties
from langbuilder.schema.validators import str_to_timestamp_validator
...
class MessageBase(SQLModel):
    ...
    def serialize_timestamp(self, value):
        ...
        if isinstance(value, str):
            # Parse string and remove timezone
    ...
    def from_message(cls, message: "Message", flow_id: str | UUID | None = None):
        ...
        if isinstance(message.timestamp, str):
            # Convert timestamp string to datetime - try multiple formats
            timestamp = None
            # List of formats to try (most specific first)
            formats = [
                "%Y-%m-%d %H:%M:%S.%f %Z",  # With microseconds and timezone name
                "%Y-%m-%d %H:%M:%S %Z",      # Without microseconds
                "%Y-%m-%d %H:%M:%S.%f",      # With microseconds, no timezone
                "%Y-%m-%d %H:%M:%S",          # Basic format
            ]
            for fmt in formats:
                try:
                    timestamp = datetime.strptime(message.timestamp, fmt).replace(tzinfo=None)
                    break
                except ValueError:
                    continue
            
            # Fallback for ISO format if strptime fails
            if timestamp is None:
                try:
                    # Remove " UTC" suffix if present for fromisoformat
                    ts_str = message.timestamp.replace(" UTC", "+00:00").replace(" ", "T")
            ...
            # Strip timezone for database storage (TIMESTAMP WITHOUT TIME ZONE)
