# Path: src/backend/langbuilder/services/database/models/flow/model.py
...
if TYPE_CHECKING:
    ...
    from langbuilder.services.database.models.publish_record.model import PublishRecord
...
class FlowBase(SQLModel):
    ...
    updated_at: datetime | None = Field(default_factory=lambda: datetime.now(timezone.utc).replace(tzinfo=None), nullable=True)
    ...
    def serialize_datetime(self, value):
        if isinstance(value, datetime):
            # For database storage, remove timezone (TIMESTAMP WITHOUT TIME ZONE)
            # For API responses, this will be handled by the response model
...
class Flow(FlowBase, table=True):  # type: ignore[call-arg]
    ...
    publish_records: list["PublishRecord"] = Relationship(back_populates="flow")
