...
from langbuilder.services.database.models.api_key.model import ApiKey, ApiKeyCreate, ApiKeyRead, UnmaskedApiKeyRead
...
async def create_api_key(session: AsyncSession, api_key_create: ApiKeyCreate, user_id: UUID) -> UnmaskedApiKeyRead:
    ...
    api_key = ApiKey(
        ...
        user_id=user_id,
        created_at=api_key_create.created_at or datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None),
    )
...
async def update_total_uses(api_key_id: UUID):
    ...
    async with session_scope() as session:
        ...
        new_api_key.last_used_at = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
