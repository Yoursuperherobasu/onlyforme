...
class PublishRecord(PublishRecordBase, table=True):  # type: ignore[call-arg]
    ...
    # Note: The unique constraint on (flow_id, platform, platform_url, status) is created
    # by migration 2a32c0912c60. We don't declare it here to avoid Alembic detecting
    # cosmetic diffs between the model and database on every startup.
    # If creating a fresh database, ensure the migration runs to create this constraint.
