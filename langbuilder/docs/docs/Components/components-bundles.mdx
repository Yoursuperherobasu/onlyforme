...
Bundles contain custom components that support specific third-party integrations with LangBuilder.
You add them to your flows and configure them in the same way as LangBuilder's core components.

## Bundle maintenance and documentation

Many bundled components are developed by third-party contributors to the LangBuilder codebase.
...
To find documentation for a specific bundled component, browse the LangBuilder docs and your provider's documentation.
...
The LangBuilder documentation focuses on using bundles within flows.
For that reason, it focuses on the LangBuilder-specific configuration steps for bundled components.
...
:::tip
The LangBuilder documentation doesn't list all bundles or components in bundles.
For the most accurate and up-to-date list of bundles and components for your version of LangBuilder, check the **Components** menu in LangBuilder.

If you can't find a component that you used in an earlier version of LangBuilder, it may have been removed or marked as a [legacy component](#legacy-components).
:::

LangBuilder offers core components in addition to third-party, provider-specific bundles.
...
<!-- Not documented but in LangBuilder as of 1.5.11 -->
<!--
* AgentQL
* Confluence
* Firecrawl
