...
LangBuilder's **Logic** components provide functionalities for routing, conditional processing, and flow management.
...
:::tip
For more examples of the **Loop** component, try the **Research Translation Loop** template in LangBuilder, or see the video tutorial [Mastering the Loop Component & Agentic RAG in LangBuilder](https://www.youtube.com/watch?v=9Wx7WODSKTo).
:::
...
The **Run Flow** component runs another LangBuilder flow as a subprocess of the current flow.
...
It was deprecated in LangBuilder version 1.1.2 and replaced by the [**Run Flow** component](#run-flow).
