...
**Embedding Model** components in LangBuilder generate text embeddings using a specified Large Language Model (LLM).

LangBuilder includes an **Embedding Model** core component that has built-in support for some LLMs.
...
You can enter component API keys directly or use LangBuilder global variables to reference your API keys.
