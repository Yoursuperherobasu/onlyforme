...
However, LangBuilder's **Agent** and **Language Model** components include built-in chat memory that is enabled by default.
...
For more information, see [Use LangBuilder agents](/agents).
...
| Is Metadata | When loading data using LangChain or LangBuilder, additional attributes may be stored under a metadata object. If the target attribute is stored this way, enable this option. It adjusts the query by generating a filter in the format: `{"metadata.<attribute_name>": "<value>"}` |
...
This component was deprecated in LangBuilder version 1.1.2.
