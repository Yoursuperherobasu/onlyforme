---
title: Contribute to LangBuilder
...
This guide is intended to help you start contributing to LangBuilder.
As an open-source codebase in a rapidly developing field, LangBuilder welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to LangBuilder, follow the [fork and pull request quickstart](https://docs.github.com/en/get-started/quickstart/contributing-to-projects).

## Install LangBuilder from source

Install LangBuilder from source by forking the repository, and then set up your development environment using Make.
...
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/langbuilder.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run LangBuilder from source

You can run LangBuilder from source after cloning the repository, even if you aren't contributing to the codebase.

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the LangBuilder directory, and then run `make run_cli`.
...
The LangBuilder frontend is served at `http://localhost:7860`.
...
To run LangBuilder from source on Windows, you can use the LangBuilder repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run LangBuilder with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run LangBuilder from the Windows Command Line:

    1. Build the LangBuilder frontend:

        ```
        cd src/frontend && npm install && npm run build && npm run start
        ```

    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/langbuilder/frontend`.
...
* To install and run LangBuilder with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run LangBuilder from a Powershell terminal:

    1. Run the following commands separately to build the LangBuilder frontend:
...
### Set up your LangBuilder development environment

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the LangBuilder development environment:
...
Since Windows doesn't include `make`, building and running LangBuilder from source uses `npm` and `uv`.

To set up the LangBuilder development environment, run the frontend and backend in separate terminals:
...
The repo includes a `.vscode/launch.json` file for debugging the backend in VSCode, which is faster than debugging with Docker Compose. To debug LangBuilder with the `launch.json` file in VSCode:
...
4. From the root of your local LangBuilder fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `langbuilder/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `langbuilder/docs/sidebars.js`.
...
* Pull request titles appear in LangBuilder's release notes, so they should explain what the PR does as explicitly as possible.
