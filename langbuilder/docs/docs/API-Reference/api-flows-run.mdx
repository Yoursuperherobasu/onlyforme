...
:::tip
LangBuilder automatically generates Python, JavaScript, and curl code snippets for the `/v1/run/$FLOW_ID` endpoint for all flows.
...
```bash
curl -X POST \
  "$LANGBUILDER_SERVER_URL/api/v1/run/$FLOW_ID" \
...
For an example of a script that extracts data from a LangBuilder API response, see the [Quickstart](/get-started-quickstart).
...
```bash
curl -X POST \
  "$LANGBUILDER_SERVER_URL/api/v1/run/$FLOW_ID?stream=true" \
...
```bash
curl -X POST \
  "$LANGBUILDER_SERVER_URL/api/v1/webhook/$FLOW_ID" \
  -H "Content-Type: application/json" \
  -H "x-api-key: $LANGBUILDER_API_KEY" \
  -d '{"data": "example-data"}'
```
