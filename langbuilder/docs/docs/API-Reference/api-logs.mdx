...
Retrieve logs for your LangBuilder flow.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your LangBuilder instance.
...
1. To enable log retrieval, include these values in your `.env` file:
    ...
    Log retrieval requires that `LANGBUILDER_LOG_RETRIEVER_BUFFER_SIZE` is greater than 0. The default value is `10000`.

2. Start LangBuilder with the updated `.env`:
...
```bash
curl -X GET \
  "$LANGBUILDER_URL/logs?lines_before=0&lines_after=0&timestamp=0" \
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/langbuilder/profile_pictures/Space.\n"
}
```
