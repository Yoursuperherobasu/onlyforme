---
title: Get started with the LangBuilder API
...
You can use the LangBuilder API for programmatic interactions with LangBuilder, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build LangBuilder as a dependency of a larger application, codebase, or service.
* Contribute to the overall LangBuilder codebase.

To view and test all available endpoints, you can access the LangBuilder API's OpenAPI specification at your LangBuilder deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the LangBuilder API in a script, see the [LangBuilder quickstart](/get-started-quickstart).
...
## Form LangBuilder API requests

While individual options vary by endpoint, all LangBuilder API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a LangBuilder API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

```bash
curl --request POST \
  --url "$LANGBUILDER_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
  --header "Content-Type: application/json" \
  --header "x-api-key: $LANGBUILDER_API_KEY" \
  --data '{
...
By default, local deployments serve the LangBuilder API at `http://localhost:7860/api`.

Remotely hosted LangBuilder deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:LANGBUILDER_PORT/api`.

You can configure the LangBuilder port number in the `LANGBUILDER_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:LANGBUILDER_PORT/api`

### Authentication

In LangBuilder versions 1.5 and later, most API endpoints require authentication with a LangBuilder API key in either an `x-api-key` header or query parameter.
...
LangBuilder API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The LangBuilder API serves `/v1` and `/v2` endpoints.
...
```bash
# Set environment variables
export LANGBUILDER_API_KEY="sk..."
export LANGBUILDER_SERVER_URL="https://localhost:7860"
...
Commonly used values in LangBuilder API requests include your [LangBuilder server URL](#base-url), [LangBuilder API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some LangBuilder API requests

Once you have your LangBuilder server URL, try calling these endpoints that return LangBuilder metadata.

### Get version

Returns the current LangBuilder API version:

```bash
curl -X GET \
  "$LANGBUILDER_SERVER_URL/api/v1/version" \
...
Returns configuration details for your LangBuilder deployment:

```bash
curl -X GET \
  "$LANGBUILDER_SERVER_URL/api/v1/config" \
  -H "accept: application/json"
  -H "x-api-key: $LANGBUILDER_API_KEY"
```
...
Returns a dictionary of all LangBuilder components:

```bash
curl -X GET \
  "$LANGBUILDER_SERVER_URL/api/v1/all" \
  -H "accept: application/json" \
  -H "x-api-key: $LANGBUILDER_API_KEY"
```
...
Because you can run LangBuilder as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the LangBuilder database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the LangBuilder codebase, you won't directly call most of the LangBuilder endpoints.
...
The following endpoints are useful for developing applications with LangBuilder and administering LangBuilder deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return LangBuilder version. See [Get version](/api-reference-api-examples#get-version).
...
You might use these endpoints when developing custom LangBuilder components for your own use or to share with the LangBuilder community:

* Develop custom components:
    * GET `/v1/all`: Return all available LangBuilder component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are for managing LangBuilder MCP servers, both LangBuilder-hosted MCP servers and external MCP server connections:
...
The following endpoints are most often used when contributing to the LangBuilder codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* Use the LangBuilder API to [run a flow](/api-flows-run).
* Use the LangBuilder API to [upload files](/api-files).
* Use the LangBuilder API to [get flow logs](/api-logs).
* Explore all endpoints in the [LangBuilder API specification](/api).