...
This tutorial shows you how to connect a JavaScript application to a [LangBuilder agent](/agents).
...
In this example, the application sends a customer's email address to the LangBuilder agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const LANGBUILDER_SERVER_ADDRESS = 'LANGBUILDER_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const LANGBUILDER_API_KEY = 'LANGBUILDER_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new LangBuilderClient({
                baseUrl: LANGBUILDER_SERVER_ADDRESS,
                apiKey: LANGBUILDER_API_KEY
            });

            console.log(`Connecting to LangBuilder server at: ${LANGBUILDER_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your LangBuilder server is running and accessible at:', LANGBUILDER_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting LangBuilder Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting LangBuilder Agent...

    Connecting to LangBuilder server at: http://localhost:7860
