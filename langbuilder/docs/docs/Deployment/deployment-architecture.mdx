---
title: LangBuilder architecture on Kubernetes
...
There are two broad types of LangBuilder deployments:
...
* **LangBuilder runtime (production)**: Deploy the LangBuilder runtime for production flows, which is headless (backend only) service focused on serving the LangBuilder API. This is used for production environments where flows are executed programmatically without the need for the visual editor. The server exposes your flows as endpoints, and then runs only the processes necessary to serve each flow.
    ...
    For information about runtime deployments on Kubernetes, see [Deploy the LangBuilder production environment on Kubernetes](/deployment-kubernetes-prod).

    :::tip
    You can start LangBuilder in headless mode with the `LANGBUILDER_BACKEND_ONLY` [environment variable](/environment-variables).
    :::

You can also [deploy the LangBuilder IDE and runtime on Docker](/deployment-docker).

## Benefits of deploying LangBuilder on Kubernetes

Deploying on Kubernetes offers the following advantages:

* **Scalability**: Kubernetes allows you to scale the LangBuilder service to meet the demands of your workload.
* **Availability and resilience**: Kubernetes provides built-in resilience features, such as automatic failover and self-healing, to ensure that the LangBuilder service is always available.
...
A typical LangBuilder deployment includes:

* **LangBuilder services**: The LangBuilder API and, for IDE deployments, the visual editor.
* **Kubernetes cluster**: The Kubernetes cluster provides a platform for deploying and managing LangBuilder and its supporting services.
* **Persistent storage**: Persistent storage is used to store the LangBuilder service's data, such as models and training data.
* **Ingress controller**: The ingress controller provides a single entry point for traffic to the LangBuilder service.
* **Load balancer**: Balances traffic across multiple LangBuilder replicas.
* **Vector database**: If you are using LangBuilder for RAG, you can integrate with the vector database in Astra Serverless.

![LangBuilder reference architecture on Kubernetes](/img/langbuilder-reference-architecture.png)

## Environment isolation

It is recommended to deploy and run separate development and production environments for LangBuilder:
...
![LangBuilder environments](/img/langbuilder-env.png)
