---
title: Deploy the LangBuilder production environment on Kubernetes
...
## Install the LangBuilder runtime Helm chart

1. Add the repository to Helm:

    ```shell
    helm repo add langbuilder https://CloudGeometry.github.io/langbuilder-helm-charts
    helm repo update
    ```

2. Install the LangBuilder app with the default options in the `langbuilder` namespace.
...
## Access the LangBuilder runtime

1. Get your service name:

    ```shell
    kubectl get svc -n langbuilder
    ```

    The service name is your release name suffixed by `-langbuilder-runtime`. For example, if you used `helm install my-langbuilder-app-with-flow`, then the service name is `my-langbuilder-app-with-flow-langbuilder-runtime`.

2. Enable port forwarding to access LangBuilder from your local machine:

    ```shell
    kubectl port-forward -n langbuilder svc/my-langbuilder-app-with-flow-langbuilder-runtime 7860:7860
    ```
...
2. Verify the secret exists:
    ...
    ```shell
    kubectl get secrets -n langbuilder openai-credentials
    ```
...
3. Upgrade the Helm release to use the secret:
    ...
    ```shell
    helm upgrade my-langbuilder-app-image langbuilder/langbuilder-runtime -n langbuilder \
      --reuse-values \
...
For non-sensitive variables, such as `LANGBUILDER_LOG_LEVEL`, you can set the value directly in `values.yaml`:

```yaml
env:
  - name: LANGBUILDER_LOG_LEVEL
    value: "INFO"
```
