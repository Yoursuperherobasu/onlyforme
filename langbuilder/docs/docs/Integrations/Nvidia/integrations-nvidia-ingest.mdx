---
title:  Integrate NVIDIA Retriever Extraction with LangBuilder
...
* The **NVIDIA Retriever Extraction** component requires the installation of additional dependencies to your LangBuilder environment. To install the dependencies in a virtual environment, run the following commands.

  * If you have the LangBuilder repository cloned and installed from source:
  ```bash
  ...
  * If you are installing LangBuilder from the Python Package Index:
  ```bash
  source **YOUR_LANGBUILDER_VENV**/bin/activate
  uv pip install --prerelease=allow 'langbuilder[nv-ingest]'
  uv run langbuilder run
  ```
