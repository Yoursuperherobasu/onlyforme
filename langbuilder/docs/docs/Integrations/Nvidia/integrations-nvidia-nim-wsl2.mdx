---
title:  Integrate NVIDIA NIMs with LangBuilder
slug: /integrations-nvidia-ingest-wsl2
---

Connect LangBuilder with NVIDIA NIM on an RTX Windows system with [Windows Subsystem for Linux 2 (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install) installed.
...
In this example, you connect a Language Model component in LangBuilder to a deployed `mistral-nemo-12b-instruct` NIM on an **RTX Windows system** with **WSL2**.
...
To connect the deployed NIM to LangBuilder, add the **NVIDIA** language model component to a flow:
