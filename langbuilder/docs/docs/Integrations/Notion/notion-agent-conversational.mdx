...
- Recommended: [Download the Conversation Agent Flow JSON](./Conversational_Notion_Agent.json), and then [import the flow](/concepts-flows-import) into LangBuilder.
...
1. Open LangBuilder and create a new flow.
2. Add the components listed above to your flow canvas, or Download the [Conversation Agent Flow](./Conversational_Notion_Agent.json)(Download link) and **Import** the JSON file into LangBuilder.
...
5. To start the flow, click the "Play" button in LangBuilder.
...
4. Review the LangBuilder logs for any error messages.

For more advanced usage and integration options, refer to the [Notion API documentation](https://developers.notion.com/) and [LangBuilder documentation](/).