...
- A [running LangBuilder server](/get-started-installation) with a [flow](/concepts-flows) that you want to trace

:::tip
If you need a flow to test the Langfuse integration, see the [LangBuilder quickstart](/get-started-quickstart).
:::
...
3. Set your Langfuse project credentials as environment variables in the same environment where you run LangBuilder.
...
## Start LangBuilder and view traces in Langfuse

1. Start LangBuilder in the same environment where you set the Langfuse environment variables:
...
2. Run a flow.
    ...
    LangBuilder automatically collects and sends tracing data about the flow execution to Langfuse.
...
To disable the Langfuse integration, remove the [Langfuse environment variables](#langfuse-credentials), and then restart LangBuilder.

## Run Langfuse and LangBuilder with Docker Compose
...
3. Add your LangBuilder credentials to your LangBuilder `docker-compose.yml` file in the `environment` section.

    The following example is based on the [example `docker-compose.yml`](https://github.com/CloudGeometry/langbuilder/blob/main/docker_example/docker-compose.yml).

    ```yml
    services:
      langbuilder:
        image: cloudgeometry/langbuilder:latest # or another version tag on https://hub.docker.com/r/cloudgeometry/langbuilder
...
5. To confirm Langfuse is connected to your LangBuilder container, run the following command:

    ```sh
    docker compose exec langbuilder python -c "import requests, os; addr = os.environ.get('LANGFUSE_HOST'); print(addr); res = requests.get(addr, timeout=5); print(res.status_code)"
    ```
