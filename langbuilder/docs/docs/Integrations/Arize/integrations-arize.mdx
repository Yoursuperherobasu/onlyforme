...
To enable Arize tracing, set the required Arize environment variables in your LangBuilder deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating LangBuilder and Arize are also available in the Arize documentation:

* [LangBuilder tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/langbuilder/langbuilder-tracing)
* [LangBuilder tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/langbuilder/langbuilder-tracing)
:::
...
2. In the root of your LangBuilder application, edit your existing LangBuilder `.env` file or create a new one.
...
4. Start your LangBuilder application with your `.env` file:
...
1. In LangBuilder, run a flow that has an **Agent** or **Language Model** component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each LangBuilder execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the LangBuilder components.
