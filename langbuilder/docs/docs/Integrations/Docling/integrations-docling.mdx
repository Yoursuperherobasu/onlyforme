...
LangBuilder integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing documents.

## Install Docling dependency

:::important
You must install the Docling dependency to use the Docling components in LangBuilder.
:::

<Tabs>
<TabItem value="oss" label="LangBuilder OSS" default>

Install the Docling extra in LangBuilder OSS with `uv pip install 'langbuilder[docling]'`.

</TabItem>

<TabItem value="desktop" label="LangBuilder Desktop">

To add a dependency to LangBuilder Desktop, add an entry for Docling to the application's `requirements.txt` file.
For more information, see [Install custom dependencies in LangBuilder Desktop](/install-custom-dependencies#langbuilder-desktop).
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + LangBuilder: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
