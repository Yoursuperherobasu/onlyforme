...
To integrate with LangBuilder, add your LangWatch API key as a LangBuilder environment variable:

1. Get a LangWatch API key from your LangWatch account.

2. Add the key to your LangBuilder `.env` file:
...
3. Restart LangBuilder with your `.env` file, if you modified the LangBuilder `.env`:
