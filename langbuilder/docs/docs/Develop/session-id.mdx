...
In the LangBuilder **Playground**, current sessions are listed on the left side of the pane.

LangBuilder uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
Replace `LANGBUILDER_SERVER_ADDRESS`, `FLOW_ID`, and `LANGBUILDER_API_KEY` with the values from your LangBuilder deployment.
```
curl --request POST \
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `langbuilder.db` with their respective `session_id` values.

## Retrieval of messages from memory by session ID

To retrieve messages from local LangBuilder memory, add a [**Message History** component](/components-helpers#message-history) to your flow.
...
Messages can be retrieved by `session_id` from the LangBuilder API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.langbuilder.org/api-monitor).

For an example of session ID in action, see [Use Session IDs in LangBuilder](https://www.youtube.com/watch?v=nJiF_eF21MY).