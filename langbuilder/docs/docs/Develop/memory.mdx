...
LangBuilder provides flexible memory management options for storage and retrieval of data relevant to your flows and your LangBuilder server.
This includes essential LangBuilder database tables, file management, and caching, as well as chat memory.

## Storage options and paths

LangBuilder supports both local memory and external memory options.

LangBuilder's default storage option is a [SQLite](https://www.sqlite.org/) database stored in your system's cache directory.
The default storage path depends on your operation system and installation method:

- **macOS Desktop**: `/Users/<username>/.langbuilder/data/database.db`
- **Windows Desktop**: `C:\Users\<name>\AppData\Roaming\com.LangBuilder\data\langbuilder.db`
- **OSS macOS/Windows/Linux/WSL (`uv pip install`)**: `<path_to_venv>/lib/python3.12/site-packages/langbuilder/langbuilder.db` (Python version may vary)
- **OSS macOS/Windows/Linux/WSL (`git clone`)**: `<path_to_clone>/src/backend/base/langbuilder/langbuilder.db`

Alternatively, you can use an external PostgreSQL database for all of your LangBuilder storage.
You can also selectively use external storage for chat memory, separate from other LangBuilder storage.
For more information, see [Configure external memory](#configure-external-memory) and [Store chat memory](#store-chat-memory).

## Local LangBuilder database tables

The following tables are stored in `langbuilder.db`:
...
• **ApiKey**: Manages LangBuilder API authentication keys. Component API keys are stored in the **Variables** table. For more information, see [API keys and authentication](/api-keys-and-authentication).
...
For more information, see the database models in the [source code](https://github.com/CloudGeometry/langbuilder/tree/main/src/backend/base/langbuilder/services/database/models).

## Configure external memory

To replace the default LangBuilder SQLite database with another database, modify the `LANGBUILDER_DATABASE_URL` environment variable, and then start LangBuilder with your `.env` file:
...
```
LANGBUILDER_DB_CONNECTION_SETTINGS='{"pool_size": 20, "max_overflow": 30, "pool_timeout": 30, "pool_pre_ping": true, "pool_recycle": 1800, "echo": false}'
LANGBUILDER_DB_CONNECT_TIMEOUT=20
```
...
- `LANGBUILDER_DB_CONNECT_TIMEOUT`: Maximum number of seconds to wait when establishing a new database connection (default: 20)

## Configure cache memory

The default LangBuilder caching behavior is an asynchronous, in-memory cache.
```
...
For other options, see the `LANGBUILDER_CACHE_TYPE` [environment variable](/environment-variables).
...
Built-in chat memory stores memories in the LangBuilder `messages` table.
...
* **Language Model and Agent components**: All messages are stored in [LangBuilder storage](#storage-options-and-paths).
...
* **Message History component**: By default, this component stores and retrieves memories from LangBuilder storage, unless you attach a **Chat Memory** component. It provides a few more options for sorting and filtering memories, although most of these options are built-in to the **Agent** component as configurable or fixed parameters.
...
Typically, this is necessary only if you have specific storage needs that aren't met by LangBuilder storage.
For example, if you want to manage chat memory data by directly working with the database, or if you want to use a different database than the default LangBuilder storage.
...
* [LangBuilder file management](/concepts-file-management)
* [LangBuilder logs](/logging)