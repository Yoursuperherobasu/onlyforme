---
title: LangBuilder TypeScript client
...
The LangBuilder TypeScript client allows your TypeScript applications to programmatically interact with the LangBuilder API.

For the client code repository, see [langbuilder-client-ts](https://github.com/datastax/langbuilder-client-ts/).

For the npm package, see [@datastax/langbuilder-client](https://www.npmjs.com/package/@datastax/langbuilder-client).

## Install the LangBuilder TypeScript package

To install the LangBuilder typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/langbuilder-client
```
...
```bash
yarn add @datastax/langbuilder-client
```
...
```bash
pnpm add @datastax/langbuilder-client
```

</TabItem>
</Tabs>

## Initialize the LangBuilder TypeScript client

1. Import the client into your code.

    ```tsx
    import { LangBuilderClient } from "@datastax/langbuilder-client";
    ```

2. Initialize a `LangBuilderClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new LangBuilderClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default LangBuilder base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## LangBuilder TypeScript client quickstart

1. With your LangBuilder client initialized, test the connection by calling your LangBuilder server.
    ...
    const client = new LangBuilderClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your LangBuilder server
    * `flowId`: The ID of the flow you want to run
    * `input`: The chat input message you want to send to trigger the flow


2. Review the result to confirm that the client connected to your LangBuilder server.

    The following example shows the response from a well-formed `runFlow` request that reached the LangBuilder server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with LangBuilder.
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments:
    ...
    For more information on streaming LangBuilder responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve LangBuilder logs with the TypeScript client

To retrieve [LangBuilder logs](/logging), you must enable log retrieval on your LangBuilder server by including the following values in your server's `.env` file:

```text
LANGBUILDER_ENABLE_LOG_RETRIEVAL=True
LANGBUILDER_LOG_RETRIEVER_BUFFER_SIZE=10000
...
async function runFlow(client: LangBuilderClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new LangBuilderClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <langbuilder.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
