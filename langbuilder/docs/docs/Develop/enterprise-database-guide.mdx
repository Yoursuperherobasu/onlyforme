---
title: LangBuilder database guide for enterprise DBAs
...
The LangBuilder database stores data that is essential for more LangBuilder operations, including startup, flow execution, user interactions, and administrative tasks.
...
This guide is designed for enterprise database administrators (DBAs) and operators responsible for deploying and managing LangBuilder in production environments.
...
## Configure LangBuilder with PostgreSQL

LangBuilder's default database is SQLite.
However, PostgreSQL is recommended for production deployments due to its scalability, performance, and robustness.

The following steps explain how to configure LangBuilder to use PostgreSQL for a standalone or containerized deployment.
...
1. Set up PostgreSQL:
   ...
   2. Create a database for LangBuilder.
   3. Create a PostgreSQL user with appropriate, minimal permissions to manage and write to the database, such as CREATE, SELECT, INSERT, UPDATE, DELETE on your LangBuilder tables.

2. Obtain the connection string in the format `postgresql://user:password@host:port/dbname`, such as`postgresql://langbuilder:securepassword@postgres:5432/langbuilder`.
...
3. Configure LangBuilder with the `.env` or `docker-compose.yml` files.

    <Tabs groupId="environment">
    <TabItem value=".env" label=".env" default>

    1. Create a `.env` file in the `langbuilder` directory:
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        LANGBUILDER_DATABASE_URL="postgresql://langbuilder:securepassword@postgres:5432/langbuilder"
        ```

    For more environment variables, see the `.env.example` file in the LangBuilder repository.

    </TabItem>
    <TabItem value="docker-compose.yml" label="docker-compose.yml">

    Use the sample `docker-compose.yml` from the LangBuilder Repository.
    ...
    services:
      langbuilder:
        image: cloudgeometry/langbuilder:latest
        ports:
          - "7860:7860"
        environment:
...
4. Start LangBuilder with your PostgreSQL connection:
...
5. Optional: Run migrations.
    ...
    LangBuilder uses migrations to manage its database schema.
    When you first connect to PostgreSQL, LangBuilder automatically runs migrations to create the necessary tables.

    Direct schema modification can cause conflicts with LangBuilder's built-in schema management.
    If you need to update the schema, you can manually run migrations with the LangBuilder CLI:

    1. Run `langbuilder migration` to preview the changes.

    2. Review the changes to ensure that it's safe to proceed with the migration.

    3. Run `langbuilder migration --fix` to run the migration and permanently apply the changes.

        This is a destructive operation that can delete data.
        For more information, see [`langbuilder migration`](/configuration-cli#langbuilder-migration).

6. To verify the configuration, create any flow using the LangBuilder visual editor or API, and then query your database to confirm the tables and activity are recorded there. The content of the flow doesn't matter; you only need to confirm that the flow is stored in your PostgreSQL database.
You can query the database in two ways:

    * Query the database container:

        ```
        docker exec -it <postgres-container> psql -U langbuilder -d langbuilder
        ```

    * Use SQL:

        ```
        SELECT * FROM pg_stat_activity WHERE datname = 'langbuilder';
        ```
...
If you have a multi-instance deployment, make sure all of your LangBuilder instances connect to the same HA PostgreSQL database.
    ...
    * Use a virtual IP or DNS name that resolves to the current primary database, such as `postgresql://langbuilder:securepassword@db-proxy:5432/langbuilder?sslmode=require`.
    * Use the provided endpoint for a managed service, such as `langbuilder.cluster-xyz.us-east-1.rds.amazonaws.com`.
...
4. Optional: Implement load balancing for read-heavy workloads:
    ...
    2. Configure LangBuilder to use a single connection string pointing to the primary PostgreSQL database or proxy.

</TabItem>
<TabItem value="AA" label="Active-Active HA">

To implement Active-Active HA, you must deploy multiple LangBuilder instances, use load balancing to distribute traffic across the instances, and ensure all instances connect to the same HA PostgreSQL database:

1. Deploy multiple LangBuilder instances using Kubernetes or Docker Swarm.

    You must configure your instances to use a shared PostgreSQL database.
    For more information, see [Best practices for LangBuilder on Kubernetes](/deployment-prod-best-practices).
...
The following example configuration is for a production deployment that has three `langbuilder-runtime` replicas, uses the Kubernetes load balancer service to distribute traffic to healthy pods, and uses the HA PostgreSQL database connection string.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: langbuilder-runtime
spec:
  replicas: 3
  selector:
    matchLabels:
      app: langbuilder-runtime
  template:
    metadata:
      labels:
        app: langbuilder-runtime
    spec:
      ...
      - name: langbuilder
        ...
        - containerPort: 7860
        env:
        - name: LANGBUILDER_DATABASE_URL
          value: "postgresql://langbuilder:securepassword@db-proxy:5432/langbuilder?sslmode=require"
---
apiVersion: v1
kind: Service
metadata:
...
LangBuilder, through [SQLAlchemy](https://docs.sqlalchemy.org/en/20/), supports reconnection attempts if `LANGBUILDER_DATABASE_CONNECTION_RETRY=True`, ensures recovery after failover, and reduces disruption once the database is back online.
...
If the PostgreSQL database becomes unavailable, the following LangBuilder functions will fail:
...
* [LangBuilder architecture on Kubernetes](/deployment-architecture)
