...
:::tip
LangBuilder provides keyboard shortcuts for the workspace.

In the LangBuilder header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
* **Core**: The categories near the top of the **Components** menu are LangBuilder's core components. They are grouped by purpose, such as **Inputs and Outputs** or **Data**. **Beta** components are newly released and still in development.
...
For information about the programmatic representation of each data type, see [LangBuilder data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal LangBuilder database. When you add a component to a flow, you create a detached copy of the component based on the information in the LangBuilder database.
These copies are detached from the primary LangBuilder database, and they don't synchronize with any updates that can occur when you upgrade your LangBuilder version.
...
When editing a flow in the workspace, LangBuilder notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of LangBuilder you are running.
