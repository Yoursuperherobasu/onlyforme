...
LangBuilder components are designed to accept and produce specific types of inputs and outputs.
...
The schema is defined in [`data.py`](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/schema/data.py).
...
The schema is defined in [`dataframe.py`](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/schema/dataframe.py).

The following attributes are available:

- **Full pandas compatibility**: All pandas DataFrame methods and functionality are supported

- **LangBuilder integration**: Accepts lists of [`Data` objects](#data), dictionaries, or existing DataFrames.
...
Because LangBuilder is built on LangChain, `LanguageModel` is actually an instance of a [LangChain chat model](https://python.langchain.com/docs/concepts/chat_models/) that uses the configuration parameters set in the originating component.
...
The `Message` schema is defined in [`message.py`](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/schema/message.py).
Some `Message` attributes have their own schema definitions, such as [`content_block.py`](https://github.com/CloudGeometry/langbuilder/blob/main/src/backend/base/langbuilder/schema/content_block.py).
...
When using the LangBuilder API, the response includes the `Message` object along with other response data from the flow run.
LangBuilder API responses can be extremely verbose, so your applications must include code to extract relevant data from the response to return to the user.
For an example, see the [LangBuilder quickstart](/get-started-quickstart).
...
For more information, see [Configure tools for agents](/agents-tools) and [Use LangBuilder as an MCP client](/mcp-client).
...
In LangBuilder, you can use <Icon name="TextSearch" aria-hidden="True" /> **Inspect output** to view the output of individual components.
