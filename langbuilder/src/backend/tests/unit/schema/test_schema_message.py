...
@pytest.fixture
def langbuilder_cache_dir(tmp_path):
    """Create a temporary langbuilder cache directory."""
    cache_dir = tmp_path / "langbuilder"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(langbuilder_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = langbuilder_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("langbuilder"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="LangBuilder")
    assert message.text == "Hello, LangBuilder!"

    prompt = message.load_lc_prompt()
    assert isinstance(prompt, ChatPromptTemplate)
    assert prompt.messages[0].content == "Hello, LangBuilder!"
...
def test_message_with_multiple_images(sample_image, langbuilder_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("langbuilder")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
...
def test_timestamp_serialization():
    ...
    # Test with timezone and fractional seconds (new format)
    msg1 = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45.123456 UTC")
    serialized1 = msg1.model_dump()
    assert serialized1["timestamp"].tzinfo == timezone.utc

    # Test with old format (still supported for backward compatibility)
    msg2 = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45 UTC")
    serialized2 = msg2.model_dump()
    assert serialized2["timestamp"].tzinfo == timezone.utc

    # Test without timezone
    msg3 = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45.123456")
    serialized3 = msg3.model_dump()
    assert serialized3["timestamp"].tzinfo == timezone.utc

    # Test that different fractional seconds result in different timestamps
    msg_early = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45.000001 UTC")
    msg_late = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45.000002 UTC")
    assert msg_early.model_dump()["timestamp"] < msg_late.model_dump()["timestamp"]

    # Test that old format (without fractional seconds) still works
    msg_old = Message(text="Test message", sender=MESSAGE_SENDER_USER, timestamp="2023-12-25 15:30:45 UTC")
    assert msg_old.model_dump()["timestamp"].tzinfo == timezone.utc
...
def cleanup():
    ...
    cache_dir = Path(user_cache_dir("langbuilder"))
    if cache_dir.exists():
        try:
