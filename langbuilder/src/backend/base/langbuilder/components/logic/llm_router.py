...
from langbuilder.inputs.inputs import BoolInput, DropdownInput, HandleInput, IntInput, MultilineInput
...
class LLMRouterComponent(Node):
    ...
    def _get_api_model_id_for_langbuilder_model(self, langbuilder_model_name: str) -> str | None:
        """Attempt to find the OpenRouter API ID for a given Langbuilder model name."""
        if not langbuilder_model_name:
            return None

        potential_names_to_check = [langbuilder_model_name, self._simplify_model_name(langbuilder_model_name)]

        if langbuilder_model_name.startswith("models/"):
            name_without_prefix = langbuilder_model_name[len("models/") :]
        ...
        elif langbuilder_model_name.startswith("community_models/"):
            name_without_prefix = langbuilder_model_name[len("community_models/") :]
            potential_names_to_check.append(name_without_prefix)
        ...
        self.log(
            f"Could not map Langbuilder model name '{langbuilder_model_name}' "
            f"(tried variants: {unique_names_to_check}) to an OpenRouter API ID."
        )
        return None

    def _get_model_specs_dict(self, langbuilder_model_name: str) -> dict[str, Any]:
        """Get a dictionary of relevant model specifications for a given Langbuilder model name."""
        if not self.use_openrouter_specs or not self._models_api_cache:
            return {
                "id": langbuilder_model_name,
                "name": langbuilder_model_name,
                "description": "Specifications not available.",
            }

        api_model_id = self._get_api_model_id_for_langbuilder_model(langbuilder_model_name)

        if not api_model_id or api_model_id not in self._models_api_cache:
            log_msg = (
                f"No cached API data found for Langbuilder model '{langbuilder_model_name}' "
...
If no model seems suitable, pick the first model in the list (index 0) as a fallback."""
    ...
    async def route_to_model(self) -> Message:
        ...
        try:
            self.log(f"Starting model routing with {len(self.models)} available Langbuilder models.")
            ...
            for i, langbuilder_model_instance in enumerate(self.models):
                langbuilder_model_name = get_model_name(langbuilder_model_instance)
                if not langbuilder_model_name:
...
Return ONLY the index number:"""
            ...
            if self._selected_model_name:
                self._selected_api_model_id = (
                    self._get_api_model_id_for_langbuilder_model(self._selected_model_name) or self._selected_model_name
                )
            else:
            ...
            specs_source = (
                ...
                else "Basic (Langbuilder model names only)"
            )
            self._routing_decision = f"""Model Selection Decision:
- Selected Model Index: {selected_index}
- Selected Langbuilder Model Name: {self._selected_model_name}
...
- Specifications Source: {specs_source}"""
            ...
            log_msg = (
                ...
                f"Langbuilder Name: '{self._selected_model_name}', API ID: '{self._selected_api_model_id}'"
            )
            self.log(log_msg)
            ...
            if self.fallback_to_first and self.models:
                ...
                self._selected_model_name = get_model_name(chosen_model_instance)
                if self._selected_model_name:
                    mapped_id = self._get_api_model_id_for_langbuilder_model(self._selected_model_name)
...
- Fallback Model Langbuilder Name: {self._selected_model_name}
...
- Reason: Automatic fallback enabled"""
    ...
    def get_selected_model_info(self) -> list[Data]:
        ...
        if self._selected_model_name:
            ...
            if "langbuilder_name" not in specs_dict:
                specs_dict["langbuilder_model_name_used_for_lookup"] = self._selected_model_name
