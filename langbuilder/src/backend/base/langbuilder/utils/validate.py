...
from langbuilder.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def validate_code(code):
    ...
    # Evaluate the function definition with langbuilder context
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            code_obj = compile(ast.Module(body=[node], type_ignores=[]), "<string>", "exec")
            try:
                # Create execution context with common langbuilder imports
                exec_globals = _create_langbuilder_execution_context()
...
def _create_langbuilder_execution_context():
    """Create execution context with common langbuilder imports."""
    context = {}

    # Import common langbuilder types that are used in templates
    try:
...
def get_default_imports(code_string):
    ...
    langbuilder_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, langbuilder_imports)
    langbuilder_module = importlib.import_module("langbuilder.field_typing")
    default_imports.update({name: getattr(langbuilder_module, name) for name in necessary_imports})
    ...
    try:
        ...
        for node in module.body:
            ...
            for base in node.bases:
                if isinstance(base, ast.Name) and any(pattern in base.id for pattern in ["Node", "LC"]):
                    return node.name
