...
from langbuilder.interface.utils import setup_llm_caching
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        from langbuilder.utils.debug_logger import clear_debug_log, get_log_path
        clear_debug_log()
        logger.info(f"Debug log cleared. Logging to: {get_log_path()}")

        # Startup message
        if version:
            logger.debug(f"Starting Langbuilder v{version}...")
        else:
            logger.debug("Starting Langbuilder...")
        ...
        try:
            ...
            lock_file = Path(tempfile.gettempdir()) / "langbuilder_starter_projects.lock"
            ...
            if "langbuilder migration --fix" not in str(exc):
                logger.exception(exc)
            raise
        finally:
            # Clean shutdown with progress indicator
            # Create shutdown progress (show verbose timing if log level is DEBUG)
            from langbuilder.__main__ import get_number_of_workers
            from langbuilder.cli.progress import create_langbuilder_shutdown_progress

            log_level = os.getenv("LANGBUILDER_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_langbuilder_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(4):
                    logger.debug("Langbuilder shutdown complete")
...
def create_app():
    ...
    if settings.mcp_server_enabled:
        from langbuilder.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Langbuilder's main.py file."""
