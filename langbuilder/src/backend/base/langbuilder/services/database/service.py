...
from langbuilder.services.database import models
...
from langbuilder.services.database.utils import Result, TableResults
from langbuilder.services.deps import get_settings_service
from langbuilder.services.utils import teardown_superuser
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        # This file is in langbuilder.services.database.manager.py
        # the ini is in langbuilder
        langbuilder_dir = Path(__file__).parent.parent.parent
        self.script_location = langbuilder_dir / "alembic"
        self.alembic_cfg_path = langbuilder_dir / "alembic.ini"
        ...
        if Path(alembic_log_file).is_absolute():
            ...
            self.alembic_log_path = Path(langbuilder_dir) / alembic_log_file
    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with self.alembic_log_path.open("w", encoding="utf-8") as buffer:
            ...
            try:
                ...
                # Check if the diffs are only cosmetic constraint changes
                # (remove_constraint + add_constraint pairs for the same constraint)
                diffs = getattr(exc, "diffs", []) if hasattr(exc, "diffs") else []
                is_cosmetic = self._is_cosmetic_constraint_diff(diffs)
                if is_cosmetic:
                    logger.debug(f"Ignoring cosmetic constraint diffs: {diffs}")
                else:
    ...
    @staticmethod
    def _is_cosmetic_constraint_diff(diffs: list) -> bool:
        """Check if the detected diffs are only cosmetic constraint changes.
        
        Returns True if all diffs are constraint-only operations that are safe to ignore:
        - Removing redundant unique constraints on primary key columns
        - Pairs of remove_constraint/add_constraint for the same constraint
        - Constraint metadata differences that don't affect actual schema
        
        These commonly occur when:
        1. Model had `unique=True` on a primary key (redundant) and it was removed
        2. Constraint naming differs between model and database
        3. PostgreSQL tracks constraints more strictly than SQLite
        """
        if not diffs:
            return True
        ...
        # If there are non-constraint operations, not cosmetic
        if other_ops:
            return False
        
        # All operations are constraint-related - these are safe to ignore:
        # 1. Only removes (removing redundant unique constraints from PKs)
        # 2. Only adds (adding constraints that already exist in DB)
        # 3. Balanced pairs (metadata differences)
        # We trust that the migrations have already created the necessary constraints
        return True
    ...
    def try_downgrade_upgrade_until_success(alembic_cfg, retries=5) -> None:
        ...
        for i in range(1, retries + 1):
            try:
                ...
                # Try downgrade, but handle empty database case
                try:
                    command.downgrade(alembic_cfg, f"-{i}")
                    # wait for the database to be ready
                    time.sleep(3)
                except util.exc.CommandError as e:
                    # "Ambiguous walk" error happens when database is empty
                    if "Ambiguous walk" in str(e):
                        logger.info("Database is empty, skipping downgrade and going straight to upgrade")
                    else:
                        raise
