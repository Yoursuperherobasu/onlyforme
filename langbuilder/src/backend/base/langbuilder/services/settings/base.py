...
from langbuilder.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from langbuilder.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default LANGBUILDER_DIR
    config_dir: str | None = None
    # Define if langbuilder db should be saved in config dir or
    # in the langbuilder directory
    save_db_in_config_dir: bool = False
    """Define if langbuilder database should be saved in LANGBUILDER_CONFIG_DIR or in the langbuilder directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Langbuilder will run in development mode."""
    database_url: str | None = None
    """Database URL for Langbuilder. If not provided, Langbuilder will use a SQLite database.
    ...
    """If True, Langbuilder will retry to connect to the database if it fails."""
    pool_size: int = 20
    ...
    Controlled by LANGBUILDER_USE_NOOP_DATABASE env variable."""
    ...
    """If set to True, Langbuilder will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Langbuilder will expose Prometheus metrics. 9090 is the default port."""
    ...
    with the same name in case Langbuilder fails to retrieve the variable value."""
    ...
    """If set to True, Langbuilder will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "localhost"
    """The host on which Langbuilder will run."""
    port: int = 7860
    """The port on which Langbuilder will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Langbuilder."""
    log_file: str | None = "logs/langbuilder.log"
    """The path to log file for Langbuilder."""
    ...
    """If set to True, Langbuilder will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Langbuilder will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Langbuilder will auto save flows."""
    ...
    """If set to False, Langbuilder will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Langbuilder will not send progress notifications in the MCP server."""
    ...
    """If set to True, Langbuilder will only partially load components at startup and fully load them on demand.
    ...
    """If set to True, Langbuilder will create starter projects. If False, skips all starter project setup.
    ...
    """If set to True, Langbuilder will update starter projects."""
    ...
    def set_dev(cls, value):
        from langbuilder.settings import set_dev
    ...
    @classmethod
    def set_langbuilder_dir(cls, value):
        if not value:
            from platformdirs import user_cache_dir

            # Define the app name and author
            app_name = "langbuilder"
            app_author = "langbuilder"

            # Get the cache directory for the application
            cache_dir = user_cache_dir(app_name, app_author)

            # Create a .langbuilder directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        if langbuilder_database_url := os.getenv("DATABASE_URL"):
            ...
            # Originally, we used sqlite:///./langbuilder.db
            ...
            from langbuilder.utils.version import is_pre_release as langbuilder_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = langbuilder_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to langbuilder directory: {database_dir}")

            pre_db_file_name = "langbuilder-pre.db"
            db_file_name = "langbuilder.db"
    ...
    def set_components_path(cls, value):
        ...
        If the `LANGBUILDER_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
                ...
                if isinstance(langbuilder_component_path, list):
                    for path in langbuilder_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {langbuilder_component_path} to components_path")
                elif langbuilder_component_path not in value:
                    value.append(langbuilder_component_path)
                    logger.debug(f"Appending {langbuilder_component_path} to components_path")
