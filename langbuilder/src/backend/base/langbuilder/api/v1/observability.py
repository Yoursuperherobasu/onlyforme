...
def fetch_traces_from_langfuse(
    ...
    Implements pagination since Langfuse API has a max limit of 100 per request.
    ...
    logger.info(f"Langfuse client type: {type(client)}")
    ...
    # Build filter kwargs for Langfuse API
    ...
    # Try the fetch_traces method with pagination (Langfuse SDK v2.x)
    if hasattr(client, 'fetch_traces'):
        try:
            ...
            while page <= max_pages:
                logger.info(f"Fetching page {page} with filters: {filter_kwargs}")
                ...
                logger.info(f"Page {page} returned {len(page_traces)} traces")
            ...
            logger.info(f"Total traces fetched with user_id filter: {len(trace_data)}")
    ...
    if hasattr(client, 'fetch_traces'):
        try:
            page = 1
            all_traces = []
            # Build fallback kwargs without user_id
            ...
            while page <= max_pages:
                logger.info(f"Fetching page {page} without user filter, limit={page_size}")
            ...
            logger.info(f"Got {len(all_traces)} total traces, filtering by user_id={user_id}")
            ...
            logger.warning(f"fetch_traces without filter failed: {e}")

    # Try direct client API with pagination
    if hasattr(client, 'client') and hasattr(client.client, 'traces'):
        try:
            logger.info(f"Attempting client.traces.list(user_id={user_id})")
...
def fetch_observations_for_trace(client, trace_id: str) -> list:
    ...
    # Try fetch_observations method (Langfuse SDK v2.x)
    if hasattr(client, 'fetch_observations'):
        try:
            ...
            if observations:
                logger.debug(f"fetch_observations got {len(observations)} for trace {trace_id}")
                return observations
        except Exception as e:
            logger.debug(f"fetch_observations failed for trace {trace_id}: {e}")

    # Try direct client observations API
    if hasattr(client, 'client') and hasattr(client.client, 'observations'):
        try:
            ...
            if hasattr(response, 'data'):
                ...
                observations = response
            if observations:
                logger.debug(f"observations.list got {len(observations)} for trace {trace_id}")
                return observations
        except Exception as e:
            logger.debug(f"observations.list API failed for trace {trace_id}: {e}")

    # Try get_observations method
    if hasattr(client, 'get_observations'):
        try:
            response = client.get_observations(trace_id=trace_id)
            if hasattr(response, 'data'):
                observations = response.data or []
            elif isinstance(response, list):
                observations = response
            if observations:
                logger.debug(f"get_observations got {len(observations)} for trace {trace_id}")
                return observations
        except Exception as e:
            logger.debug(f"get_observations failed for trace {trace_id}: {e}")
...
async def get_langfuse_status() -> LangfuseStatusResponse:
    ...
    try:
        ...
        return LangfuseStatusResponse(
            connected=True,
            host=host,
            message="Langfuse connected successfully"
        )
    except Exception as e:
        return LangfuseStatusResponse(
            connected=False,
...
async def get_user_traces(
    ...
    try:
        ...
        for trace in raw_traces:
            ...
            # Fetch observations for this trace to get accurate metrics
            try:
                observations = fetch_observations_for_trace(client, str(trace_id))
                parsed_obs = [parse_observation(obs) for obs in observations]
            except Exception as e:
                logger.debug(f"Failed to fetch observations for trace {trace_id}: {e}")
                parsed_obs = []
...
async def get_trace_detail(
    ...
    try:
        ...
        if trace_user_id != str(current_user.id):
            raise HTTPException(status_code=404, detail="Trace not found")
...
async def get_user_sessions(
    ...
    - search: Search sessions by ID or trace name
    """
    client = get_langfuse_client()
    if not client:
        raise HTTPException(status_code=503, detail="Langfuse not configured.")
    ...
    try:
        ...
        for trace in raw_traces:
            ...
            # Fetch observations for accurate metrics
            try:
                observations = fetch_observations_for_trace(client, str(trace_id))
                parsed_obs = [parse_observation(obs) for obs in observations]
            except:
            ...
            sessions_data[session_id]["models"].update(models)
            if timestamp:
...
async def get_user_metrics(
    ...
    Includes model breakdown, daily usage, and performance metrics.

    Filter Parameters:
    - from_date: Start date filter (YYYY-MM-DD format)
    - to_date: End date filter (YYYY-MM-DD format)
    - search: Search traces by name
    ...
    try:
        user_id = str(current_user.id)
        logger.info(f"Fetching metrics for user_id: {user_id}")
        ...
        if from_date:
            try:
                from_timestamp = datetime.strptime(from_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
                logger.info(f"Filtering from: {from_timestamp}")
            except ValueError:
                raise HTTPException(status_code=400, detail="Invalid from_date format. Use YYYY-MM-DD")
        if to_date:
            try:
                ...
                logger.info(f"Filtering to: {to_timestamp}")
            except ValueError:
                raise HTTPException(status_code=400, detail="Invalid to_date format. Use YYYY-MM-DD")
        ...
        if models:
            ...
            logger.info(f"Filtering models: {model_filter}")
            ...
            for obs in parsed_obs:
                ...
                if model_filter and obs.model:
                    if obs.model not in model_filter:
                        continue  # Skip observations from non-matching models
...
def extract_flow_project_info(trace) -> tuple[str | None, str | None, str | None, str | None]:
    """
    Extract flow_id, flow_name, project_id, project_name from trace metadata/tags.

    Returns: (flow_id, flow_name, project_id, project_name)
    """
    ...
    if not flow_id or not project_id:
        for tag in tags:
            if isinstance(tag, str):
                if tag.startswith('flow_id:') and not flow_id:
                    flow_id = tag.split(':', 1)[1]
    ...
    # Fallback: use trace name as flow_id if not found
    if not flow_id:
        flow_id = get_attr(trace, 'name')

    return flow_id, flow_name, project_id, project_name
...
def _match_trace_to_flow(trace, flows_by_id: dict, flows_by_name: dict) -> tuple[str | None, str | None]:
    ...
    Returns: (flow_id, flow_name) or (None, None) if no match
    """
    ...
    # Method 1: Check metadata for flow_id (most reliable)
    flow_id_from_meta = metadata.get('flow_id')
    if flow_id_from_meta and flow_id_from_meta in flows_by_id:
        flow = flows_by_id[flow_id_from_meta]
        return str(flow.id), flow.name

    # Method 2: Check tags for flow_id
    for tag in tags:
        if isinstance(tag, str) and tag.startswith('flow_id:'):
            fid = tag.split(':', 1)[1]
...
async def get_user_agents(
    ...
    try:
        ...
        flows_result = await session.exec(
            select(Flow).where(Flow.user_id == current_user.id, Flow.is_component == False)
        )
        ...
        # Build flow_id to folder mapping
        ...
        for trace in raw_traces:
            flow_id, flow_name = _match_trace_to_flow(trace, flows_by_id, flows_by_name)
            if flow_id:
                session_id = get_attr(trace, 'session_id', 'sessionId')
                if session_id:
                    session_to_agent[session_id] = (flow_id, flow_name)
                if flow_id not in agent_flow_traces:
                    agent_flow_traces[flow_id] = []
                agent_flow_traces[flow_id].append(trace)
        ...
        for trace in raw_traces:
            ...
            matched_flow_id, matched_flow_name = _match_trace_to_flow(trace, flows_by_id, flows_by_name)
            if matched_flow_id:
                flow_id, flow_name = matched_flow_id, matched_flow_name
            # Otherwise, check if it's part of an agent's session (vertex traces)
            elif session_id and session_id in session_to_agent:
                flow_id, flow_name = session_to_agent[session_id]
            ...
            if matched_flow_id:
                agents_data[flow_id]["trace_count"] += 1

            # But aggregate tokens/cost from ALL traces (including vertex)
            agents_data[flow_id]["total_tokens"] += trace_tokens
            agents_data[flow_id]["total_cost"] += trace_cost
            agents_data[flow_id]["models"].update(models)
            agents_data[flow_id]["latencies"].extend(latencies)
            agents_data[flow_id]["error_count"] += error_count
            if session_id:
                agents_data[flow_id]["sessions"].add(session_id)
            if timestamp:
                agents_data[flow_id]["timestamps"].append(timestamp)
...
async def get_user_projects(
    ...
    try:
        ...
        # Fetch all traces
        raw_traces = fetch_traces_from_langfuse(client, user_id, limit=500)
